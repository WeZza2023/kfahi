...
import 'ContentWritingtest.dart';

class ContentwritingPage extends StatefulWidget {
  ContentwritingPage(
  ...
  @override
  _ContentwritingPageState createState() => _ContentwritingPageState();
}

class _ContentwritingPageState extends State<ContentwritingPage> {
  ...
  Future<void> _playNextVid() async {
    ...
    if (snapshot.exists) {
      ...
      if (widget.videoNum < ContentWritingVids.length - 1 &&
          doneLecs.length != ContentWritingVids.length) {
        Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => ContentwritingPage(
                videoNum: widget.videoNum + 1,
                videoId: ContentWritingVids[widget.videoNum + 1],
              ),
            ));
      } else {
        Navigator.pushReplacement(context,
            MaterialPageRoute(builder: (context) => Contentwritingtest()));
      }
    }
  }
